...
- `2026-09-23`: 补齐悬停提示：对象行显示来源模型与权重摘要、置信度、外接框、属性、描述与锁定/困难/隐藏；文件行显示路径、标注文件、复核状态与时间（当前图片另给模型/人工/未记录拆分）；面板把手与折叠键、缩放框、两个筛选器、标签搜索、亮度对比度滑块、自动标注面板按钮都补上说明。状态点的含义并入行提示，不再与它抢同一个属性。
...
- 悬停详情：对象行显示类别、形状、产出模型与权重摘要、置信度、顶点数与外接框、属性、描述以及保护状态（锁定/困难/隐藏）；文件行显示完整路径、标注文件、复核状态与时间，当前图片还会给出模型框/人工框/未记录的拆分。浮动面板把手与折叠键、缩放框、两个筛选器、亮度对比度滑块同样都有说明。
