...
class TestSettingsController(unittest.TestCase):
    ...
    def test_debounce_save_and_flush(self):
        # Pick the option that differs from the current value so the update
        # is an actual change (template default may already be "modelscope").
        current = self.controller.get_value("model_hub")
        other = next(v for v in ("github", "modelscope") if v != current)
        self.controller.update_field("model_hub", other)
