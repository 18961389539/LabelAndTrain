...
class TestCanvasAdjustmentWidget(unittest.TestCase):
    ...
    def test_brightness_contrast_updates_are_throttled(self):
        ...
        # Poll instead of a single tight qWait: the 33ms timer can be
        # coalesced past a 43ms wait on coarse Windows timer resolution.
        deadline = 2000
        while len(signal_spy) < 1 and deadline > 0:
