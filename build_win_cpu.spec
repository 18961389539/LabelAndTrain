# -*- mode: python -*-
# vim: ft=python
# Windows CPU build spec (derived from packaging/pyinstaller/specs/x-anylabeling-win-cpu.spec)
# - icon switched to .ico (Windows)
# - BUNDLE (.app) removed (macOS-only)
# ROOT_DIR is resolved from X_ANYLABELING_ROOT, so this file may live anywhere.
...
exe = EXE(
    ...
    a.zipfiles,
    a.datas,
    name=f'JLLabelingAndTrain-v{__version__}-CPU',
    debug=False,
    strip=False,
    upx=False,
    runtime_tmpdir=None,
