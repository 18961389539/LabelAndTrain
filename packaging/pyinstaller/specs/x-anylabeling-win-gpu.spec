...
# vim: ft=python
#
# Windows GPU build (onedir) — bundles torch/ultralytics so the built-in
# training panel works in the packaged app. Derived from x-anylabeling-win-cpu.spec.
#
# Key differences vs the CPU spec:
#   * does NOT exclude torch / torchvision / ultralytics / nvidia
#   * onedir layout (EXE + COLLECT) — a 4GB+ onefile would re-extract on every
#     launch and is far more fragile
#   * exe name ends with "-GPU"
#   * icon uses the Windows .ico file
...
sys.setrecursionlimit(10000)  # required on Windows
...
a = Analysis(
    ...
    hiddenimports=[
        ...
        'matplotlib.mathtext',
        # Training is entered via a string __import__ in app.py; make sure the
        # worker entrypoint and its heavy deps are always analysed.
    ...
    excludes=[
        # WebEngine / PDF are not used and save ~500MB.
        'PyQt6.QtWebEngineWidgets', 'PyQt6.QtWebEngineCore',
        'PyQt6.QtWebEngineQuick', 'PyQt6.QtWebChannel',
        'PyQt6.QtPdf', 'PyQt6.QtPdfWidgets', 'PyQt6.QtWebSockets',
        # A foreign Qt binding must never sneak in.
        'PySide6', 'PySide6.scripts',
        # torchaudio is not installed; triton (Linux inductor) not used.
        'torchaudio', 'triton',
    ],
)
...
exe = EXE(
    ...
    name=f'JLLabelingAndTrain-v{__version__}-GPU',
    debug=False,
    strip=False,
    upx=False,
    console=False,
