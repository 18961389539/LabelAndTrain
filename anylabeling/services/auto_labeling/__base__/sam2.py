...
class SegmentAnything2ONNX:
    ...
    def __init__(
        self, encoder_model_path, decoder_model_path, device, on_stage=None
    ) -> None:
        self.encoder = SAM2ImageEncoder(
            encoder_model_path, device, on_stage=on_stage
        )
...
def _create_session(path, device, on_stage=None):
    ...
    Args:
        ...
        on_stage: Optional callable receiving human-readable progress
            messages. Session creation on a 340MB encoder can take a
            minute or more on CPU — without these messages the UI looks
            frozen.
    ...
    if on_stage is not None:
        on_stage(
            QCoreApplication.translate(
                ...
                "(this can take 1-2 minutes on first load)...",
            )
        )
    ...
    if on_stage is not None:
        on_stage(
            QCoreApplication.translate(
                "Model", "Inference engine initialized."
            )
        )
    return session
...
class SAM2ImageEncoder:
    def __init__(self, path: str, device: str, on_stage=None) -> None:
        # Initialize model
        self.session = _create_session(path, device, on_stage=on_stage)
