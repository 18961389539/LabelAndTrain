...
class ExportManager:
    def __init__(self):
        ...
        # Redirector used as sys.stdout/stderr *inside the export worker*.
        # It must be owned here (not created ad-hoc in the worker) so the UI
        # can connect its log_signal once and actually receive ultralytics
        # export progress instead of silently swallowing it.
        self.log_redirector = ExportLogRedirector()
    ...
    def _export_worker(self, weights_path: str, export_format: str):
        try:
            ...
            # Reuse the manager-owned redirector whose log_signal the UI has
            # already connected, so export output reaches the log view.
            log_redirector = self.log_redirector
