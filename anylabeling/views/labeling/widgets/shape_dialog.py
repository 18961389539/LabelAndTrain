...
class ShapeModifyDialog(RangeTableDialog):
    ...
    def accept(self):
        ...
        # Preview the actual files that will be affected so an accidental
        # range typo does not wipe a hundred files "blind".
        preview = self._range_file_preview(start_idx, end_idx)
        if preview:
            template = self.tr(
                "Are you sure you want to perform '%s' on frames %s to %s?\n\n%s"
            )
            confirm_msg = template % (
                operation_name,
                start_idx,
                end_idx,
                preview,
    ...
    def _range_file_preview(self, start_idx, end_idx):
        """Human-readable preview of the files affected by a range op.

        Returns a multi-line string like::

            img_0031.json ... img_0150.json  (120 annotation files)

        or an empty string when no files can be resolved.
        """
    ...
    def delete_annotations_in_range(self, start_idx, end_idx):
        ...
        for i in range(start_idx - 1, end_idx):
            ...
            if os.path.exists(label_file):
                try:
                    # Move to _delete_ instead of os.remove so the file can
                    # still be recovered after a mistaken range.
                    save_file = os.path.join(
                        save_path, os.path.basename(label_file)
                    )
                    if os.path.exists(save_file):
                        stem, ext = os.path.splitext(save_file)
                        save_file = f"{stem}_{i}{ext}"
                    shutil.move(label_file, save_file)
                    deleted_count += 1
