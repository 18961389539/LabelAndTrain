...
from .canvas_rotation import (  # noqa: F401
    CURSOR_DEFAULT,
    CURSOR_DRAW,
    CURSOR_GRAB,
    CURSOR_MOVE,
...
class Canvas(
    BrushCanvasMixin, CanvasRotationMixin, QtWidgets.QWidget
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Set mask opacity options (default 40% opaque = 102/255).
        self.mask_opacity = self.mask_config.get("opacity", 102)
    ...
    def redo_shape(self):
        """Re-apply the most recently undone state (Redo).

        Mirrors restore_shape(): the re-applied state is pushed onto the
    ...
    def _undo_pending_edge_point(self):
        ...
        if len(self.shapes_backups) >= 2 and shape in self.shapes:
            ...
            # Dropping the backup of a cancelled edit invalidates any
            # pending redo branch as well.
            self.shapes_redo_backups.clear()
    ...
    def _paint_empty_state(self):
        """Fill the canvas while the overlay CTA is shown on the viewport."""
        t = get_theme()
        p = self._painter
        p.begin(self)
        p.fillRect(self.rect(), QtGui.QColor(t["background"]))
        p.end()

    def _paint_content(self):
        """Paint the loaded pixmap and all annotations."""
        p = self._painter
        p.begin(self)
    ...
    def wheelEvent(self, ev: QWheelEvent):
        ...
        # Rectangle wheel-editing only when the user explicitly holds Alt,
        # so a plain wheel always scrolls the viewport and never silently
        # resizes a selected box (common accidental damage during browsing).
        if (
            self.editing()
            ...
            and (mods & QtCore.Qt.KeyboardModifier.AltModifier)
