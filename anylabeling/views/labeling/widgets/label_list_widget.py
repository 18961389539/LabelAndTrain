...
class LabelListWidget(QtWidgets.QListView):
    ...
    def __init__(self):
        ...
        # NOTE: Keep all one-time setup out of paintEvent. Connecting here
        # (or in paintEvent) once per paint accumulates unbounded duplicate
        # signal connections, making every selection change slower and
        # eventually freezing the UI.
    ...
    def paintEvent(self, event):
        """Draw a placeholder when the label list is empty."""
        ...
        if self.model() is None or self.model().rowCount() == 0:
            ...
            p.setFont(font)
            p.setPen(QtGui.QColor(t["text_placeholder"]))
            p.drawText(
                self.rect(),
