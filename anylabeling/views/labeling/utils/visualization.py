...
# Mapping from internal shape-type IDs to their Chinese display label.
# The IDs themselves are part of the on-disk format and must NOT change,
# but the UI should show localized labels — so we translate at the
# checkbox construction site instead of touching Shape.get_supported_shape().
_SHAPE_DISPLAY_ZH = {
    "polygon": "多边形",
    "rectangle": "矩形",
    "rotation": "旋转矩形",
...
def save_visualization(self):
    ...
    options = _show_visualization_dialog(self)
    if options is None:
        return
...
def _show_visualization_dialog(self):
    ...
    for index, shape_type in enumerate(Shape.get_supported_shape()):
        display = _SHAPE_DISPLAY_ZH.get(shape_type, shape_type)
    ...
    image_range = "all" if all_files_radio.isChecked() else "current"
